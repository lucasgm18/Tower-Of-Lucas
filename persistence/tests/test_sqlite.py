...
TEMP_DB_PATH = "saves/temp_unittest.db"
...
class TestSQLitePersistence(unittest.TestCase):
    ...
    def test_save_and_load_character_sqlite(self) -> None:
        ...
        save_character(warrior, db_path=TEMP_DB_PATH)

        loaded = load_character(hero_name, db_path=TEMP_DB_PATH)
    ...
    def test_list_saved_characters_sqlite(self) -> None:
        ...
        save_character(c1, db_path=TEMP_DB_PATH)
        save_character(c2, db_path=TEMP_DB_PATH)

        saved = list_saved_characters(db_path=TEMP_DB_PATH)
    ...
    def test_delete_character_sqlite(self) -> None:
        ...
        save_character(hero, db_path=TEMP_DB_PATH)

        self.assertTrue(delete_character(hero_name, db_path=TEMP_DB_PATH))
        self.assertIsNone(load_character(hero_name, db_path=TEMP_DB_PATH))
        self.assertFalse(delete_character(hero_name, db_path=TEMP_DB_PATH))
